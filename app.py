...
def main():
    with st.sidebar:
        st.header("TOMATO DISEASE DETECTION HERE")
    ...
    if image_file == None:
        ...
        if st.button("Process"):
            ...
            if (prediction_cls(model.predict(img))) == 'Tomato Bacterial spot':
                ...
                st.write(
                    "Hot water treatment can be used to kill bacteria on and in seed. For growers producing their own seedlings, avoid over-watering and handle plants as little as possible. Disinfect greenhouses, tools, and equipment between seedling crops with a commercial sanitizer.")
                ...
                st.subheader("Remedies :")
                st.write(
                    "Most spider mites can be controlled with insecticidal/miticidal oils and soaps. The oils—both horticultural oil and dormant oil—can be used. Horticultural oils can be used on perennial and woody ornamentals during the summer but avoid spraying flowers, which can be damaged.")
                ...
                st.subheader("Remedies :")
                st.write(
                    "There's no way to treat a plant with tomato spotted wilt virus. However, there are several preventative measures you should take to control thrips—the insects that transmit tomato spotted wilt virus. Weed, weed, and weed some more. Ensure that your garden is free of weeds that thrips are attracted to.")
